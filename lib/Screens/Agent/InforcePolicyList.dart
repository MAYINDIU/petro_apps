...
class InforcePolicy {
  ...
  factory InforcePolicy.fromJson(Map<String, dynamic> json) {
...
class InforcePolicyApiResponse {
  final String agentName;
  final String empId;
  final List<InforcePolicy> policies;

  InforcePolicyApiResponse({
    required this.agentName,
    required this.empId,
    required this.policies,
  });

  factory InforcePolicyApiResponse.fromJson(Map<String, dynamic> json) {
    final policyList = (json['policyList'] as List? ?? [])
        .map((item) => InforcePolicy.fromJson(item))
        .toList();

    return InforcePolicyApiResponse(
...
// --- 2. API Service ---
class InforcePolicyApiService {
  static const String _apiUrl = "https://nliapi.nextgenitltd.com/api/policy-list";

  Future<InforcePolicyApiResponse> fetchInforcePolicies() async {
    ...
    if (response.statusCode == 200) {
      ...
      if (jsonResponse['success'] == true) {
        return InforcePolicyApiResponse.fromJson(jsonResponse);
      } else {
        throw Exception(jsonResponse['message'] ?? 'Failed to load in-force policies.');
      }
    } else {
...
// --- 3. UI Widget ---
class InforcePolicyListPage extends StatefulWidget {
  const InforcePolicyListPage({super.key});

  @override
  State<InforcePolicyListPage> createState() => _InforcePolicyListPageState();
}

class _InforcePolicyListPageState extends State<InforcePolicyListPage> {
  late Future<InforcePolicyApiResponse> _apiFuture;
  List<InforcePolicy> _allPolicies = [];
  List<InforcePolicy> _filteredPolicies = [];
  ...
  void initState() {
    super.initState();
    _apiFuture = InforcePolicyApiService().fetchInforcePolicies();
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text('In-force Policy List'),
      ...
      body: Column(
        children: [
          _buildFilterSection(),
          Expanded(
            child: FutureBuilder<InforcePolicyApiResponse>(
              future: _apiFuture,
              builder: (context, snapshot) {
                ...
                if (_filteredPolicies.isEmpty) {
                  return const Center(child: Text('No policies match your criteria.'));
                }
                return ListView.builder(
                  padding: const EdgeInsets.all(8.0),
                  itemCount: _filteredPolicies.length,
                  itemBuilder: (context, index) => _buildPolicyExpansionTile(_filteredPolicies[index]),
                );
              },
            ),
          ),
          if (_allPolicies.isNotEmpty) _buildFooter(),
        ],
      ),
    );
  }
        ...
        children: [
          TextFormField(
            ...
            decoration: InputDecoration(
              hintText: 'Search by Name or Policy No...',
  ...
  Widget _buildDateFilter(String label, DateTime? date, Function(DateTime) onDateSelected) {
    return ElevatedButton.icon(
      onPressed: () async {
        final pickedDate = await showDatePicker(context: context, initialDate: date ?? DateTime.now(), firstDate: DateTime(2000), lastDate: DateTime(2101));
        if (pickedDate != null) onDateSelected(pickedDate);
      },
      icon: const Icon(Icons.calendar_today, size: 14),
      label: Text(date == null ? label : DateFormat('dd-MM-yy').format(date), style: const TextStyle(fontSize: 12)),
      style: ElevatedButton.styleFrom(backgroundColor: kAccentBlue, foregroundColor: Colors.white, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(8))),
    );
  }
  ...
  Widget _buildProjectFilter() {
    return Container(
      ...
      decoration: BoxDecoration(color: kAccentBlue, borderRadius: BorderRadius.circular(8)),
      child: DropdownButtonHideUnderline(
        child: DropdownButton<String>(
          ...
          onChanged: (value) => setState(() { _selectedProject = value; _applyFilters(); }),
        ),
      ),
    );
  }

  Widget _buildPolicyExpansionTile(InforcePolicy policy) {
    return Card(
      elevation: 3,
      ...
      child: ExpansionTile(
        ...
        trailing: Column(
          ...
          children: [
            Text(_currencyFormatter.format(double.tryParse(policy.sumAssured) ?? 0), style: const TextStyle(color: kSuccessColor, fontWeight: FontWeight.bold)),
            Text('Next Due: ${policy.nextPaymentDate}', style: const TextStyle(color: Colors.grey, fontSize: 12)),
          ],
        ),
        children: [
          Padding(
            ...
            child: Column(
              children: [
                ...
                _buildDetailRow('Premium Due:', _currencyFormatter.format(double.tryParse(policy.premiumDue) ?? 0)),
    ...
    return Padding(
      ...
      child: Row(
        ...
        children: [
          ...
          Text(value, style: const TextStyle(fontWeight: FontWeight.w500, color: kTextColorDark)),
        ],
      ),
    );
  }
