...
// --- Data Model ---
class AreaBusinessMonthlyTopData {
  ...
  factory AreaBusinessMonthlyTopData.fromJson(Map<String, dynamic> json) {
    double toDouble(dynamic value) {
      if (value is num) return value.toDouble();
      return double.tryParse(value?.toString() ?? '0.0') ?? 0.0;
    }

    return AreaBusinessMonthlyTopData(
      areaCode: json['area_code']?.toString() ?? 'N/A',
...
class AreaBusinessMonthlyTopFirstYearScreen extends StatefulWidget {
  const AreaBusinessMonthlyTopFirstYearScreen({Key? key}) : super(key: key);

  @override
  _AreaBusinessMonthlyTopFirstYearScreenState createState() =>
      _AreaBusinessMonthlyTopFirstYearScreenState();
}

class _AreaBusinessMonthlyTopFirstYearScreenState
    extends State<AreaBusinessMonthlyTopFirstYearScreen> {
  List<AreaBusinessMonthlyTopData> _currentMonthItems = [];
  List<AreaBusinessMonthlyTopData> _previousMonthItems = [];
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
        ...
        if (jsonResponse['success'] == true && jsonResponse['branch'] is List) {
          ...
          for (var itemJson in filteredData) {
            final item = AreaBusinessMonthlyTopData.fromJson(itemJson);
